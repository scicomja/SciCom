...
export const authorizedPostMultipartForm = async (url, form, jwt) => {
	...
	Object.keys(form).forEach(field => {
		...
		if (_.isArray(form[field])) {
			form[field].forEach(ele => formData.append(field, ele))
			return
		}
...
export const getLatestProjects = async token => {
	return await authorizedGet(`${serverURL}/project/latest`, token)
}

export const changePassword = async (info, token) => {
	return await authorizedPost(`${serverURL}/auth/changePassword`, info, token)
}
...
export const createProject = async (project, token) => {
	return await authorizedPostMultipartForm(
		`${serverURL}/project`,
		project,
		token
	)
...
export const acceptApplication = async (appId, token) => {
	return await authorizedPost(
		`${serverURL}/application/${appId}/accept`,
		null,
		token
	)
}
...
export const rejectApplication = async (appId, token) => {
	return await authorizedPost(
		`${serverURL}/application/${appId}/reject`,
		null,
		token
	)
}
...
export const completeProject = async (projectId, token) => {
	return await authorizedPost(
		`${serverURL}/project/complete/${projectId}`,
		null,
		token
	)
}
...
export const cvURL = ({ username, CV }) => `${serverURL}/user/${CV}.pdf`
