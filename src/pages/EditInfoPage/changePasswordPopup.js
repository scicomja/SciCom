...
class ChangePasswordPopup extends React.Component {
	...
	constructor(props) {
		...
		this.labels = _.pick(Locale.changePasswordPrompt, [
			"originalPassword",
			"newPassword",
			"confirmPassword"
		])
	...
	render() {
		...
		return (
			<Modal isOpen={visible} toggle={onClose}>
				<ModalHeader>{Locale.changePasswordPrompt.title}</ModalHeader>
				<ModalBody>
					...
					<Formik
						...
						{({ values, errors, dirty, isValid, setFieldValue }) => {
							return (
								<Form>
									{Object.keys(this.labels).map(name => (
										<FormGroup key={name}>
											<Label for={name}>{this.labels[name]}</Label>
											<Input
												type="password"
												name={name}
												tag={Field}
									...
									<div style={style.buttonGroup}>
										<Button
											...
											{Locale.changePasswordPrompt.changePasswordButton}
										</Button>
										<Button onClick={onClose}>
											{Locale.changePasswordPrompt.cancelButton}
										</Button>
									</div>
								</Form>
							)
